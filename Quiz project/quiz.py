...
try:
    ...
    messagebox.showerror('Database Failed', 'Failed to Connect with Database'+str(e))
    sys.exit(0)
...
def saveToFile(html ,filename): #It takes html code
    ...
    with open(filename , "w" , encoding='utf-8') as f:
        f.write(html)
    messagebox.showinfo('Congrats', 'Certificate Generate. Filename:' + filename)
...
def generateCertificate(name,correct,wrong,total,percent,grade):
    ...
    filename='{}-{}.html'.format(name,date)
    html='''
    <html>
  ...
  {{
    ...
    '''.format(name,category,date,correct,wrong,total,percent,grade)
